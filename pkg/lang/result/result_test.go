...
var anErr = errors.New("E!R!R!O!R!")
...
func TestRIfOK(t *testing.T) {
	assert.Panics(t, func() { OK(10).IfOK(func(int) { panic(0) }) })
	assert.NotPanics(t, func() { Err[int](anErr).IfOK(func(int) { panic(0) }) })
}

func TestRIfErr(t *testing.T) {
	assert.NotPanics(t, func() { OK(10).IfErr(func(error) { panic(0) }) })
	assert.Panics(t, func() { Err[int](anErr).IfErr(func(error) { panic(0) }) })
}
...
func TestJSON(t *testing.T) {
	...
	{ // Bidirect
		...
		assert.NotNil(t, err)
		assert.Equal(t, value.Zero[R[int]](), after1)

		var after2 R[float64]
		err = json.Unmarshal(bs, &after2)
		assert.NotNil(t, err)
		assert.Equal(t, value.Zero[R[float64]](), after2)
	...
	{ // Unmarshal empty: `{}`
		...
		assert.Nil(t, err)
		assert.Equal(t, value.Zero[R[string]](), r)
	}
...

	{
		...
		assert.NotNil(t, err)
		assert.Equal(t, value.Zero[R[bool]](), r)
	}
	{
		// Unmarshal illegal: ``
...
func TestROptional(t *testing.T) {
	assert.Equal(t, of.OK(1), OK(1).Option())
	assert.Equal(t, of.Nil[int](), Err[int](anErr).Option())
}
