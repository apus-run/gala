...
import (
	...
	"github.com/apus-run/gala/components/cache/lru"
)
...
func TestCache_SetWithEmptyKey(t *testing.T) {
	store := lru.New()
	defer store.Close()
...
func TestCache_SetWithNilValue(t *testing.T) {
	store := lru.New()
	defer store.Close()
...
func TestCache_Conn(t *testing.T) {
	store := lru.New()
	defer store.Close()
