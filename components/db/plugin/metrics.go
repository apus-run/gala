...
// MetricsPlugin 是一个为GORM实现Prometheus指标监控的插件。
...
// NewMetricsPlugin 创建并初始化一个新的GORM指标插件。
...
func (p *MetricsPlugin) before(operation string) func(*gorm.DB) {
...
// 所有操作都可以复用此函数。
func (p *MetricsPlugin) recordMetrics(db *gorm.DB, operation string) {
