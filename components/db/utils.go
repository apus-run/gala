...
func MaybeAddInToWhere[T any](b *WhereBuilder, fieldIn []T, column string, opt ...func(builder *WhereBuilder)) {
	...
	values := sliceutils.Map(fieldIn, func(f T) any { return f })
...
// Exists checks if any record exists in the database.
...
// LimitAndOffset applies limit and offset to the database query.
