...
func MakeTestConnection(t *testing.T) (mg *mongo.Client, coll *mongo.Collection, teardown func()) {
	collName := fmt.Sprintf("test_%d", time.Now().Nanosecond())
	return MakeTestConnectionWithColl(t, "mongodb://root:example@localhost:27017/", collName)
}
...
func MakeTestConnectionWithColl(t *testing.T, uri, cname string) (mg *mongo.Client, coll *mongo.Collection, teardown func()) {
	log.Print("[DEBUG] connect to mongo test instance")
	opts := options.ClientOptions{}
	opts.SetAppName("test")
	opts.SetConnectTimeout(time.Second)
	mg, err := mongo.Connect(context.Background(), options.Client().ApplyURI(uri))
...
func TestWriter(t *testing.T) {
	count := func(coll *mongo.Collection) (res int) {
