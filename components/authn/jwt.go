...
func NewJwtAuth(store rdb.Storer, opts ...Option) *JwtAuth {
	options := Apply(opts...)
	return &JwtAuth{
		Options: options,
		store:   store,
...
func (j *JwtAuth) Sign(ctx context.Context) (Token, error) {
	now := time.Now()
...
func (j *JwtAuth) callStore(fn func(rdb.Storer) error) error {
